...
transformValues = function(ectypeMappings){
  var transformedVals = {};

  for (var val in ectypeMappings){
    if (val === "_hooks") {continue};
    transformedVals[val] = ectypeMappings[val]();
  }
...
ectypes.createProxy = function(typeName, mapping, strategyFunctionName, hooks){
  var proxy = function(){
    var obj = ectypes.strategy[strategyFunctionName](typeName, transformValues(mapping));
