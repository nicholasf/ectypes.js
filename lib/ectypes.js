	...
	this.types.forEach(function(plan){	
		...
		for (var mapping in plan){
			...
			for (var prop in ectypes.strategy) {
				...
				if (!ignoring){
					ectypes[mapping][prop] =  ectypes.createProxy(plan[mapping], prop, hooks)
				}
			}
...
ectypes.createProxy = function(mapping, strategyFunctionName, hooks){
  var transformedVals = {};
  ...
  return function(){
    var obj = ectypes.strategy[strategyFunctionName](mapping, transformedVals);
